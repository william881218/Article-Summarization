...
class Article():
    def __init__(self, article, word2index, index2vec, idx, predicting=False):
        ...
        self.text_size = len(article['text']) + 1
        text = [word2index[word] for word in article['text']] + [EOS_ID]
        if self.text_size > 300:
            text = text[0: max_input_len]
            text[max_input_len - 1] = EOS_ID
        else:
            text += [PAD_ID for _ in range(max(0, max_input_len - self.text_size))]
        text = [index2vec[word] for word in text]
        self.text = torch.from_numpy(np.array(text).astype('float32'))

        self.summary_size = 0 if predicting else len(article['summary']) + 1
        self.summary = None
        if not predicting:
            summary = [word2index[word] for word in article['summary']] + [EOS_ID]
            if len(summary) > max_input_len:
                summary = summary[0: max_input_len]
                summary[max_input_len - 1] = EOS_ID
            else:
                summary += [PAD_ID for _ in range(max(0, max_input_len - self.summary_size))]
...
class ArticleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if article.summary == None:
            return article.text, article.text_size, idx
        else:
            return article.text, article.summary, idx, article.text_size, article.summary_size
